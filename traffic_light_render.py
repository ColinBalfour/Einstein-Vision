...
def _mk_disc(parent, tag, z_offset):
    ...
    bpy.ops.mesh.primitive_cylinder_add(radius=0.08, depth=0.015)
    disc              = bpy.context.object
    disc.name         = f"{parent.name}_{tag}"
    disc.parent       = parent
    disc.location     = (0.0, 0.12, z_offset)   # tweak forward offset if needed
    disc.rotation_euler = (math.radians(90), 0, 0)  # face the street
...
def _ensure_bulbs(parent):
    ...
    if not {'red', 'yellow', 'green'} <= existing:
        _mk_disc(parent, "red",    0.10)
        _mk_disc(parent, "yellow", 0.05)
        _mk_disc(parent, "green",  0.00)
...
def highlight_light_color(traffic_obj, colour):
    ...
    for child in traffic_obj.children:
        ...
        if tag in mapping:
            ...
            on  = (tag == colour)
            _ensure_emission(mat, mapping[tag], strength = 25 if on else 0.2)
...
def add_direction_arrow(traffic_obj, direction):
    ...
    bpy.ops.mesh.primitive_cylinder_add(radius=0.04, depth=0.01)
